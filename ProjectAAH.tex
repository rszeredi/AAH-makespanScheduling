...
\todo[inline]{Note: when $n=10$ and $m=10$ the solution is optimal!! But we get a huge makespan gap, so need to mention that the makespan gap is not a great measure when $n$ and $m$ are comparable.}
...
\todo[inline]{why $m=2$ and very large processing times}
\todo[inline]{explain how we generate fat tailed distribution}
...
We also find that when the instance size became exceedingly large, using a uniform distribution of processing times (in blue) made the simulated annealing algorithm find close to optimal solutions more easily.  \\

\todo[inline]{Explain evening out of makespan gap results, it becomes really small for large n and small m}
\todo[inline]{Explain results in Table \ref{tab:Q3Pathoresults} and \ref{fig:Q3Patho}}
In an attempt to create input instance with worst case processing times we tried another distribution.\todo[inline]{explain bad distribution choice}


\todo[inline]{The makespan gap is not good at determining the optimal solution when n is close to m, see n=10 and m=10 always gives makespan gap around 70-80, also for the pathological instances we considered!!! However, this could actually be close to the optimal solution....}
